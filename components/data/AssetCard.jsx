...
export function AssetCard({ symbol, name, pair, price, delta, series = [], currency = '$', suffix, priceLabel = 'Price', menuLabel, onMenu, holding, actions, style, ...rest }) {
    ...
    <Card interactive glow={tone} padding="var(--pad-card)" style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-6)', ...style }} {...rest}>
      ...
      <div style={{ display: 'flex', gap: 'var(--sp-5)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-2)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-4)', marginTop: 2 }}>
            <DeltaChip value={delta} showIcon style={{ alignSelf: 'flex-start' }} />
            {holding}
          </div>
        </div>
      </div>
